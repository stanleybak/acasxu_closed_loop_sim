...
def get_time_elapse_mat(command1, dt, command2=0):
    ...
    dtheta1 = (y1 / 180 * math.pi)
    dtheta2 = (y2 / 180 * math.pi)
...
def state7_to_state5(state7):
    ...
    return np.array([rho, theta, psi, v_own, v_int], dtype=float)
...
class State():
    ...
    def __init__(self, init_vec):
        ...
        self.min_dist = -1, np.inf, np.zeros(0) # (time, distance, state)
    ...
    def step(self):
        ...
        # linear space variables:
        # x1, y1, vx1, vy1, x2, y2, vx2, vy2

        x1 = self.vec[0]
        y1 = self.vec[1]
        vx1 = math.cos(self.vec[2]) * self.v_own
        vy1 = math.sin(self.vec[2]) * self.v_own

        x2 = self.vec[3]
        y2 = self.vec[4]
        vx2 = math.cos(self.vec[5]) * self.v_int
        vy2 = math.sin(self.vec[5]) * self.v_int

        start_vec = np.array([x1, y1, vx1, vy1, x2, y2, vx2, vy2], dtype=float)

        time_elapse_mat = get_time_elapse_mat(self.command, State.dt, intruder_cmd)

        s = time_elapse_mat @ start_vec

        # extract observation (like theta) from state
        new_time = self.vec[-1] + State.dt
        theta1 = math.atan2(s[3], s[2])
        theta2 = math.atan2(s[7], s[6])
        dyn_state = [s[0], s[1], theta1, s[4], s[5], theta2, new_time]
        self.vec = np.array(dyn_state, dtype=float)
    ...
    def simulate(self, cmd_list):
        ...
        prev_dist = self.min_dist[1]

        while t + 1e-6 < tmax:
            self.step()

            cur_dist = math.sqrt((self.vec[0] - self.vec[3])**2 + (self.vec[1] - self.vec[4])**2)

            if cur_dist < self.min_dist[1]:
                self.min_dist = t, cur_dist, self.vec.copy()
    ...
    def update_command(self):
        ...
        rho, theta, psi, v_own, v_int = state7_to_state5(self.vec)
...
def main():
    ...
    for seed in range(10000):
        if seed % 1000 == 0:
            print(f"{seed//1000}", end='', flush=True)
        elif seed % 100 == 0:
        ...
        state5 = state7_to_state5(init_vec)
        ...
        # reject simulations where the minimum distance was at the random start point
        if s.min_dist[0] < 1.0:
            continue

        # save most interesting state based on some criterea
        if interesting_state is None or np.sum(s.commands) > np.sum(interesting_state.commands):
            interesting_seed = seed
            interesting_state = s

    print(f"\nplotting most interesting state with seed {interesting_seed}")

    # optional plot
    assert interesting_state is not None
    plot(interesting_state, save_mp4=False)
